    ...
    return (
      <div className="min-h-screen bg-[#F7F5FB] text-[#26233D]">
        ...
        <main className="mx-auto flex min-h-[60vh] max-w-xl flex-col items-center justify-center px-4 text-center">
          ...
          <Link href="/numerology" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-sm font-bold text-white hover:bg-violet-700">
  ...
  return (
    <div className="min-h-screen bg-[#F7F5FB] text-[#26233D] antialiased">
      ...
      <main className="mx-auto max-w-6xl px-3.5 py-6 sm:px-6 sm:py-10">
        ...
        <section className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm sm:rounded-3xl sm:p-8">
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-600">Personal number profile</p>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {Object.entries(NUMEROLOGY_NUMBER_LABELS).map(([key, label]) => {
              ...
              return (
                <article key={key} className="rounded-2xl border border-violet-100 bg-[#F8F7FC] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-[11px] font-bold uppercase tracking-wide text-violet-800">Planet: {result.planet}</p>
                    </div>
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-violet-700 text-lg font-black text-white">{result.number}</span>
                  </div>
                  <p className="mt-4 border-t border-violet-100 pt-3 text-xs leading-relaxed text-slate-700">{result.meaning}</p>
                </article>
              );
            })}
          </div>
        ...
        {report.planetPositions && (
          <section className="mt-5 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm sm:rounded-3xl sm:p-8">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-600">Personal birth chart</p>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {Object.entries(report.planetPositions).map(([planet, position]) => {
                ...
                return (
                  <article key={planet} className="rounded-xl border border-violet-100 bg-[#F8F7FC] p-4">
        ...
        {['lifePath', 'personality', 'expression'].map((key) => {
          ...
          return (
            <section key={key} className="mt-5 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm sm:rounded-3xl sm:p-8">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-600">{NUMEROLOGY_NUMBER_LABELS[key]} insight</p>
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div className="rounded-xl bg-violet-50 p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wide text-violet-800">Strengths</h3>
                  <ul className="mt-2 space-y-1 text-sm text-slate-700">{content.strengths.map((item) => <li key={item}>• {item}</li>)}</ul>
                </div>
                <div className="rounded-xl bg-indigo-50 p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wide text-indigo-900">Growth edges</h3>
        ...
        <section className="mt-5 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm sm:rounded-3xl sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-violet-600">Growth patterns</p>
            ...
            <CheckCircle2 className="hidden h-6 w-6 shrink-0 text-violet-600 sm:block" />
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {CHALLENGE_LABELS.map(([key, label]) => {
              const result = report.results[key];
              return (
                <article key={key} className="rounded-xl border border-violet-100 bg-[#F8F7FC] p-4">
        ...
        <section className="mt-5 rounded-2xl border border-violet-200 bg-violet-50 p-5 shadow-sm sm:rounded-3xl sm:p-8">
