...
import { ChevronDown, ChevronRight, Menu, MessageCircle, Sparkles, User, Users, X } from 'lucide-react';
...
// A grouped section label, e.g. "EXPLORE" — matches the reference design's
// small-caps section headers above each card group.
...
// One row in a card group: icon in a circle, label, chevron. Matches the
// reference design's "Consultations / Horoscope" style rows.
...
export default function Navbar({
  ...
  // Same for the mobile-menu profile dropdown (separate state/ref since
  // both buttons technically exist in the DOM at once, just shown/hidden
  // responsively).
  useEffect(() => {
  ...
  const handleLogout = async () => {
    ...
    // State cleanup happens in the onAuthStateChange listener above, so it
    // works the same way regardless of where logout was triggered.
  };
      ...
      <div className="mx-auto max-w-6xl px-3 sm:px-6">
        <div className="relative flex min-h-14 items-center justify-between gap-4 sm:min-h-[64px]">
          ...
          <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-6 text-[13px] font-semibold text-slate-700 md:flex">
            ...
            <Link href="/blogs" className="relative py-2 transition-colors hover:text-amber-800">
              Blog
              <span className="absolute inset-x-0 bottom-0 h-0.5 origin-left scale-x-0 rounded-full bg-violet-600 transition-transform hover:scale-x-100" />
            </Link>
          </nav>
          ...
          <div className="relative hidden items-center md:flex" ref={profileRef}>
            <button
              ...
              className={`inline-flex h-9 w-9 items-center justify-center rounded-full border transition ${authedEmail
        ...
        {/* Mobile slide-out menu — redesigned as grouped card sections */}
        <div
          aria-hidden={!isOpen}
        ...
        >
          <nav
            className={`fixed right-0 top-0 flex h-dvh w-[min(86vw,340px)] flex-col overflow-y-auto border-l border-violet-200 bg-[#F7F5FB] px-4 pb-6 pt-4 shadow-[-18px_0_50px_rgba(76,29,149,0.18)] transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
          ...
          >
              ...
              <div className="flex items-center gap-2">
                <div className="relative" ref={mobileProfileRef}>
                  <button
                    ...
                    className={`inline-flex h-10 w-10 items-center justify-center rounded-xl border transition ${authedEmail
                      ? 'border-violet-200 bg-violet-100 text-violet-700 hover:bg-violet-200'
            ...
            {/* Grouped card sections — matches the reference design. */}
            ...
            <div className="space-y-2">
              <MenuCard href="/blogs" label="Blog" icon={MessageCircle} onClick={() => setIsOpen(false)} />
            </div>
          </nav>
        </div>
      </div>
      ...
      {showAuthModal && (
        <AuthModal
          ...
          onSuccess={(user) => {
            ...
            // No chart to resume here — this is a plain navbar login. If the
            // person came from an astrologer page mid-flow, that page's own
            // AuthModal instance (and its onAuthStateChange listener) is
            // what triggers generateChart(); this one is purely for
            // account access from anywhere else on the site.
          }}
        />
      )}
    </header>
