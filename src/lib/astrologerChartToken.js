...
const TOKEN_SECRET = process.env.ASTROLOGER_CHART_TOKEN_SECRET
  || process.env.CAREER_CHART_TOKEN_SECRET
  || (process.env.NODE_ENV === 'production' ? '' : 'development-astrologer-chart-secret');
const TOKEN_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;

function getTokenSecret() {
  if (!TOKEN_SECRET) throw new Error('Astrologer chart token secret is not configured.');
  return TOKEN_SECRET;
}
...
export function createAstrologerChartToken(chart) {
  const payload = Buffer.from(JSON.stringify({ chart, createdAt: Date.now() })).toString('base64url');
  return `${payload}.${sign(payload)}`;
}

export function readAstrologerChartToken(token) {
